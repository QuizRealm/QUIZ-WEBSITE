/* ==========================================================================
   ...
   --------------------------------------------------------------------------
   • Fix: Removed IIFE wrapper for ES Module compatibility.
...
// We assume window.db and window.auth are set by firebase-config.js
// If not, we wait for them in init()
...
export const GameEngine = {
    ...
    // EXPOSE TRACK EVENT (For Host Game)
    trackEvent(eventName, data) {
        this.log(`Event: ${eventName}`, data);
        // Integrate with ux-core logic if needed
    }
};
    ...
    async syncDown() {
        ...
        try {
            ...
            if (snap.exists()) {
                ...
                LocalManager.save();
                GameEngine._recalculateLevel(false);
            }
        } catch (e) {}
    },
...
if (typeof window !== 'undefined') {
    setTimeout(() => { if (!window.GameEngine) GameEngine.init(); }, 100);
}